...
public class UserConfig {
    ...
    public static void saveConfig(boolean withFile, File oldFile) {
        synchronized (sync) {
            try {
                SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences(ApplicationLoader.instance_number + "userconfing", Context.MODE_PRIVATE);
...
//                dialogsLoadOffsetAccess = -1;
                ...
                if (!notificationsConverted) {
                    try {
                        ...
                        preferences = ApplicationLoader.applicationContext.getSharedPreferences(ApplicationLoader.instance_number + "Notifications", Context.MODE_PRIVATE);
