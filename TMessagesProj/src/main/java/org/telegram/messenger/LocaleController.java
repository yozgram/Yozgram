...
import org.telegram.Adel.shamsi.ShamsiCalendar;
import org.telegram.Adel.shamsi.ShamsiDate;
...
//        languagesDict.put(localeInfo.shortName, localeInfo);
    ...
    private void saveOtherLanguages() {
        SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences(ApplicationLoader.instance_number + "langconfig", Activity.MODE_PRIVATE);
    ...
    public static String formatDateChat(long date) {
        try {
            ...
            if (!language.toLowerCase().equals("fa")) {
                ...
                ShamsiDate dateToShamsi = ShamsiCalendar.dateToShamsi(new Date(date));
                if (!(Math.abs(System.currentTimeMillis() - date) < 31536000000L)) {
                    return dateToShamsi.toDateString();
                }
                StringBuilder append = new StringBuilder().append(dateToShamsi.getDay()).append(" ");
                language = language.toLowerCase().equals("fa") ? ShamsiCalendar.getShamsiMonth(dateToShamsi.getMonth()) : ShamsiCalendar.getShamsiMonthEn(dateToShamsi.getMonth());
    ...
    public static String formatDateOnline(long date) {
        try {
	        ...
	        if (!language.toLowerCase().equals("fa")) // Adel
	        {
		        Calendar rightNow = Calendar.getInstance();
		        int      day      = rightNow.get(Calendar.DAY_OF_YEAR);
		        int      year     = rightNow.get(Calendar.YEAR);
...
//			        return String.format("%s %s", LocaleController.getString("LastSeenDate", R.string.LastSeenDate), format);
			        ...
			        StringBuilder stringBuilder = new StringBuilder().append(dateToShamsi.getDay()).append(" ");
			        String format = ShamsiCalendar.getShamsiMonth(dateToShamsi.getMonth());
			        Object[] objArr2 = new Object[2];
			        objArr2[0] = stringBuilder.append(format).toString();
			        objArr2[1] = getInstance().formatterDay.format(new Date(date));
			        format = formatString("formatDateAtTime", R.string.formatDateAtTime, objArr2);
			        Object[] objArr = new Object[2];
			        objArr[0] = getString("LastSeenDate", R.string.LastSeenDate);
			        objArr[1] = format;
...
//			        return String.format("%s %s", LocaleController.getString("LastSeenDate", R.string.LastSeenDate), format);
			        ...
			        Object[] objArr = new Object[2];
			        objArr[0] = dateToShamsi.toDateString();
			        objArr[1] = getInstance().formatterDay.format(new Date(date));
    ...
    public static String stringForMessageListDate(long date) {
        try {
	        ...
	        if (language.toLowerCase().equals("fa")) // Adel
	        {
		        ...
		        {
//			        return getInstance().formatterYear.format(new Date(date));
			        return dateToShamsi.toDateString(); // Adel
		        }
		        else
		        {
...
//				        return getInstance().formatterWeek.format(new Date(date));
                        ...
                        rightNow.setTimeInMillis(date);
                        return ShamsiCalendar.getShamsiWeekDaysMap().get(rightNow.get(Calendar.DAY_OF_WEEK));
			        }
			        else
			        {
//				        return getInstance().formatterMonth.format(new Date(date));
                        ...
                        // Adel
                        StringBuilder stringBuilder = new StringBuilder().append(dateToShamsi.getDay()).append(" ");
                        String format = language.toLowerCase().equals("fa") ? ShamsiCalendar.getShamsiMonth(dateToShamsi.getMonth()) : ShamsiCalendar.getShamsiMonthEn(dateToShamsi.getMonth());
