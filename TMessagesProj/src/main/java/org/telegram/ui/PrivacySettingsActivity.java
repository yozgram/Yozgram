        ...
        listView.setOnItemClickListener(new RecyclerListView.OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                ...
                if (position == blockedRow) {
                    ...
                    ApplicationLoader.applicationContext.getSharedPreferences(ApplicationLoader.instance_number + "mainconfig", Activity.MODE_PRIVATE).edit().putInt("secretWebpage2", MessagesController.getInstance().secretWebpagePreview).commit();
                    if (view instanceof TextCheckCell) {
                        ((TextCheckCell) view).setChecked(MessagesController.getInstance().secretWebpagePreview == 1);
                    }
                } else if (position == callsP2PRow) {
                    SharedPreferences prefs=getParentActivity().getSharedPreferences(ApplicationLoader.instance_number + "mainconfig", Context.MODE_PRIVATE);
